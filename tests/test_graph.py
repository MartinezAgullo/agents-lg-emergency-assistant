...
def print_state_summary(state: GraphState, step: str):
    ...
    if state.get("dangers"):
        ...
        for danger in state["dangers"]:
            severity = (
                f" [{danger.severity}]"
                if hasattr(danger, "severity") and danger.severity
                else ""
            )
    ...
    if state.get("risk_assessments"):
        ...
        medium_risks = [
            r for r in state["risk_assessments"] if r.risk_level == "medium"
        ]
        ...
        if high_risks:
            ...
            for risk in high_risks:
                print(
                    f"     • {risk.asset_id} ← {risk.danger_id} ({risk.distance_km:.2f} km)"
                )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ Loaded {len(actors_data['assets'])} assets and {len(actors_data['dangers'])} dangers"
        )
    ...
    try:
        ...
        with SqliteSaver.from_conn_string(checkpoint_path) as checkpointer:
            ...
            for event in graph_with_checkpoint.stream(
                ...
                if step_count == 1:
                ...
                elif event.get("proposed_plan") and not event.get(
                    "evaluation_feedback"
                ):
                    ...
                    event.get("final_plan")
                    or event.get("notifications_sent") is not None
                ):
        ...
        if final_state.values.get("final_plan"):
            ...
            print(
                f"   - Notifications sent: {'Yes' if final_state.values.get('notifications_sent') else 'No'}"
            )
