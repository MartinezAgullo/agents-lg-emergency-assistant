...
SUSPICIOUS_KEYWORDS = [
    ...
    "jailbreak", "prompt", "instruction", "system", "admin",
    ...
    for i, asset in enumerate(data.get("assets", [])):
        ...
        for field in REQUIRED_ASSET_FIELDS:
            if field not in asset:
                return False, f"Asset '{asset.get('id', i)}' missing required field: '{field}'"
    ...
    for i, danger in enumerate(data.get("dangers", [])):
        ...
        for field in REQUIRED_DANGER_FIELDS:
            if field not in danger:
                return False, f"Danger '{danger.get('id', i)}' missing required field: '{field}'"
    ...
    for asset in data.get("assets", []):
        for field in ["description", "comments"]:
            if field in asset and asset[field]:
                ...
                if not is_safe:
                    all_issues.extend([f"Asset '{asset['id']}' {field}: {issue}" for issue in issues])
    ...
    for danger in data.get("dangers", []):
        for field in ["description", "comments"]:
            if field in danger and danger[field]:
                ...
                if not is_safe:
                    all_issues.extend([f"Danger '{danger['id']}' {field}: {issue}" for issue in issues])
...
def validate_input(raw_input: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    if not is_safe:
        return False, f"[FIREWALL] Prompt injection detected:\n" + "\n".join(issues), {}
    ...
    for asset in raw_input.get("assets", []):
        ...
        if not is_valid:
            return False, f"[FIREWALL] Invalid coordinates in asset '{asset['id']}': {error}", {}
    ...
    for danger in raw_input.get("dangers", []):
        ...
        is_valid, error = _check_coordinate_validity(lat, lon)
        if not is_valid:
            return False, f"[FIREWALL] Invalid coordinates in danger '{danger['id']}': {error}", {}
    ...
    for danger in raw_input.get("dangers", []):
        if "severity" in danger and danger["severity"]:
            ...
            if severity_lower not in ["low", "medium", "high"]:
                return False, f"[FIREWALL] Invalid severity '{danger['severity']}' in danger '{danger['id']}'. Must be: low, medium, or high", {}
